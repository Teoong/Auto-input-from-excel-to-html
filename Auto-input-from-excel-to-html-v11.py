...
print("=" * 60)
print("          DYNAMIC MATRIX CONTROLLER (SHEET SELECT)")
print("=" * 60)
...
try:
    ...
    if selected_sheet not in wb.sheetnames:
        ...
        print("Please check for typos or capitalization and run the script again.")
        sys.exit(1)
        
    sheet = wb[selected_sheet]  # Force selection of the requested sheet tab
    ...
    # Loop exactly the number of required cycles specified by the user
    for current_cycle in range(requested_cycles):
        ...
        # --- INNER LOOP: Copies 8 cells horizontally across the current row ---
        for col_idx in range(1, 9):  # Columns 1 to 8 (A to H)
            ...
            time.sleep(0.1)  # Micro delay to allow browser to digest text string
            
            # 2. Browser Focus Navigation Logic:
            # Check if we just filled out the 8th column cell (Column H)
            if col_idx == 8:
                # Only hit the 7 extra TABs if this is NOT the absolute last cycle in your request
                if current_cycle < (requested_cycles - 1):
                    print("      ⌨️ Row end reached. Pressing TAB 7 times to bypass buttons...")
                    for _ in range(7):
                        pyautogui.press('tab')
                        time.sleep(0.05)
            else:
                # Standard step forward between columns 1 through 7
                pyautogui.press('tab')
                
            time.sleep(0.1)  # Micro stabilization pause

        print(f"   ✓ Completed Cycle for Excel Row {excel_row}.")
